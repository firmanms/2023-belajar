        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
          ...
          <div class="p-6 text-gray-900 dark:text-gray-100">
            ...
            <div class="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              @php
              $iduser=$row->user_id;
              $nama_user= \App\Models\User::where('id',$iduser)->get();
              // dd($namauser);
              @endphp
                  <h5 class="ml-0 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><u>{{ $namauser->name }}</u> :  {{ $row->komentar }}</h5>
                  ...
                  @php
                  $iduser2=$val->user_id;
                  $nama_user2= \App\Models\User::where('id',$iduser2)->get();
                  @endphp 
                  <h5 class="ml-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><u>{{ $namauser2->name }}</u>  :  {{ $val->komentar }}</h5>
                  @endforeach
